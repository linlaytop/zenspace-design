...
### 寺庙佛教设计方案
寺庙佛教设计团队将线性 RGBW 像素条完美隐藏于幕墙竖肋型材内部，平时完全隐形。每晚 20:00-22:30，幕墙自动演绎「数据流动」主题的动态流光。22:30 后自动退晕至 15% 亮度，深夜 00:00 后仅保留 5% 安全照明。
...
### 文物保护之难
中国现存元代以前古建筑仅 427 处，每一处都是不可再生的文化瑰宝。传统灯光安装需要在木构上打孔固定，这对古建的寿命是致命的。李明远会长三年来拒绝了 17 家照明企业的方案，直到遇见寺庙佛教设计。

### 寺庙佛教设计突破
寺庙佛教设计的专利「无痕重力抱箍系统」利用古建自有结构受力，通过精密计算抱箍与木材的接触压强（<0.15 MPa），确保数十年不伤木骨。同时选用 2200K 超低色温，将蓝光辐射（损伤彩画的主因）降至仪器无法检测的水平。
...
### 暗天空保护
斐济莫米湾是全球最佳的暗夜保护区之一。喜多度假村（Six Senses）作为全球顶尖的生态度假品牌，将「零光害」写入了品牌标准。寺庙佛教设计团队历时 14 个月，与天文学家、生态学家联合研发，打造出全球首个「暗天空三星认证」的度假村照明系统。
...
export const WORKFLOW_STEPS = [
  {
    step: 1,
    title: "需求沟通与场地勘察",
    desc: "深入了解建筑载体、环境生态、业主审美偏好和预算范围。寺庙佛教设计勘察团队携带 Dialux 现场采点设备、光谱分析仪、暗天空合规检测仪，进行为期 1-3 天的全方位现场勘察。",
...
《场地勘察报告》——包含 30+ 项参数指标、120+ 张现场照片、5 年期光照变化模拟动画。`,
  ...
  },
  {
    ...
    desc: "基于现场数据生成照度模拟、色温规划和效果预览渲染。寺庙佛教设计创意团队运用 Dialux 4.0 和 Relux 专业级光模拟软件，生成 1:1 真实施工级照度模拟图。",
...
### 汇报演示
寺庙佛教设计独创「沉浸式方案汇报系统」——在会议室搭建 1:1 实景光环境模拟空间，让业主提前「走进」完工后的夜景效果。`,
  ...
  },
  {
    step: 3,
    title: "深化设计与灯具选型",
    desc: "精确到每颗灯的型号、角度、色温与安装方式的工程级图纸。寺庙佛教设计深化设计团队由 12 位平均从业 14 年的资深照明工程师组成，确保每一张施工图都可直接指导现场安装。",
...
- 施工配合：全程驻场工程师`,
  ...
  },
  {
    step: 4,
    title: "非侵入式施工安装",
    desc: "采用专利抱箍与压载工艺，对古建零损伤，对幕墙零破坏。寺庙佛教设计自有施工团队均持有「古建筑照明施工资质证书」，平均从业 9 年，是中国最资深的照明施工力量。",
...
- 业主手机 APP 实时查看施工进度`,
  ...
  },
  {
    step: 5,
    title: "调光与 DMX 中控编程",
    desc: "16-Bit 对数调光曲线编写、多时段场景编排与暗天空合规验证。寺庙佛教设计中控编程团队由 8 位 DMX 国际认证工程师组成，累计编程通道数超过 120 万。",
...
- 暗天空合规验收（上射光通量占比 <1.5%）`,
  ...
  },
  {
    step: 6,
    title: "验收交付与长期运维",
    desc: "IDA 暗天空合规认证、操作培训、远程运维及年度光衰校准。寺庙佛教设计提供「5+5」双五年的全周期运维承诺，确保灯光系统在 10 年内始终保持设计效果。",
    fullArticle: `## 第六步：验收交付与长期运维

### 交付文档
- 《寺庙佛教设计照明系统操作手册》（含 iPad 一键控制指南）
...
### 客户满意度
寺庙佛教设计交付项目客户满意度：99. 7%（2020-2025 年均）`,
            ...
            <div className="flex flex-col gap-0 relative">
              ...
              <div className="mt-3 p-4 bg-cyan-950/10 border border-cyan-500/15 rounded-xl flex items-center gap-3">
                ...
                <p className="text-xs text-neutral-300 leading-relaxed">
                  从初勘到验收，寺庙佛教设计标准交付周期为 <b className="text-cyan-400">4-8 周</b>。
